...
//APIs
import { getObExceptionals } from "services/common.service"
import {
...
import { getAllOriginationApproval } from "services/origination.service"
...
const ApprovalDetails = props => {
  ...
  const verifyOnBoardingApprovals = () => {
    if (
      originationApproval.requestDtoList !== null &&
      originationApproval.requestDtoList !== undefined &&
      originationApproval.approvalStepDto !== null &&
      originationApproval.approvalStepDto !== undefined
    ) {
      return (
        originationApproval.requestDtoList.filter(item => item.status === "P")
          .length > 0 &&
        originationApproval.approvalStepDto.workflowStep.name === "AG_LEVEL_2"
      )
    }
  }
  ...
  const onSubmitObApproval = () => {
    var value = document.getElementById(
      `ob_comment_${obExceptionalIndex}`
    ).value
    if (value === "") {
      document
        .getElementById(`ob_error_comment_${obExceptionalIndex}`)
  ...
  const onSubmitCaApprove = (index, item) => {
    ...
    if (value === "") {
      document
        .getElementById(`ca_error_comment_${index}`)
        .classList.remove("d-none")
      return
    }
  ...
  return (
    <Row>
                                  ...
                                  <NavItem key={index}>
                                    ...
                                    >
                                      <div className="d-flex justify-content-between align-items-center">
                                        <p className="m-0">
                                          0{index + 1}.{" "}
                                          {getObExceptionals(item.type)}
                                        </p>
                                        {item.status === "A" && (
                                          <i className="bx bx-check-square font-size-20" />
                                        )}
                                        {item.status === "R" && (
                                          <i className="bx bx-x-circle font-size-20" />
                                        )}
                                      </div>
                                    </NavLink>
                                  </NavItem>
                                          ...
                                          {item.status === "P" &&
                                            ...
                                            "AG_LEVEL_2" && (
                                              <div className="form-group mt-3 d-flex justify-content-end align-items-center">
                                                <button
                                                  onClick={() =>
                                                    handleObExceptionalRejectClickOpen(
                                                      index,
                                                      item
                                                    )
                                                  }
                                                  className="btn btn-danger w-md me-2"
                                                >
                                                  <SyncLoader
                                                    loading={isLoadingOb}
                                                  >
                                                ...
                                                <button
                                                  onClick={() =>
                                                    handleObExceptionalClickApproveOpen(
                                                      index,
                                                      item
                                                    )
                                                  }
                    ...
                    >
                      <div className="accordion-body">
                        <Row>
                                  ...
                                  steps.map((item, index) => (
                                    <li key={index} className="event-list mb-3">
                                      ...
                                      <div className="d-flex">
                                        <div className="me-3">
                                          {item && item.stepAction === "A" && (
                                            <i className="bx bxs-check-circle h1 text-success" />
                                          )}
                                          {item && item.stepAction === "R" && (
                                            <i className="bx bxs-log-out-circle h1 text-warning" />
                                          )}
                                          {item && item.stepAction === "J" && (
                                            <i className="bx bxs-x-circle h1 text-danger" />
                                          )}
                                          {item && item.stepAction === "P" && (
                                            <i className="bx bxs-hourglass-top h1 text-info" />
                                          )}
                                        </div>
                                          ...
                                          <div>
                                            ...
                                            <div className="d-flex justify-content-between align-items-top">
                                              ...
                                              {item.lastModifiedBy !== null &&
                                                item &&
                                                item.stepAction !== "P" && (
                                                  <div className="d-flex justify-content-between align-items-center font-size-12">
                          ...
                          {verifyActiveStepAndUser() == true &&
                            verifyOnBoardingApprovals() == false &&
                            verifyExceptionalApprovals() == false && (
                              <Col md="12">
      ...
      >
        ...
        <DialogActions>
          <Button
            style={{ color: "white", backgroundColor: "#34c38f" }}
            onClick={createActionApprovalStep}
            autoFocus
      ...
      >
        <DialogTitle
          ...
          style={{ color: "#DC4C64", alignContent: "center" }}
        >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {rejectType == "J"
              ? "Are you sure do you want to reject this appriasal?"
              : ""}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
            style={{ color: "white", backgroundColor: "#DC4C64" }}
            onClick={createActionApprovalRejectStep}
            autoFocus
            disabled={isButtonDisabled}
          >
            Reject
          </Button>
          <Button style={{ color: "gray" }} onClick={handleRejectClose}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions>
          <Button
            style={{ color: "white", backgroundColor: "#34c38f" }}
            onClick={onSubmitCaApproval}
            autoFocus
            disabled={isButtonDisabled}
          >
            {"Approve"}
          </Button>
          <Button style={{ color: "gray" }} onClick={handleCaClose}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions>
          <Button
            style={{ color: "white", backgroundColor: "#DC4C64" }}
            onClick={onSubmitCaRejection}
            autoFocus
            disabled={isButtonDisabled}
          >
            Reject
          </Button>
          <Button style={{ color: "gray" }} onClick={handleCaRejectClose}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions>
          <Button
            style={{ color: "white", backgroundColor: "#34c38f" }}
            onClick={onSubmitObApproval}
            autoFocus
            disabled={isButtonDisabled}
          >
            {"Approve"}
          </Button>
          <Button style={{ color: "gray" }} onClick={handleObExceptionalClose}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            style={{ color: "gray" }}
            onClick={handleObExceptionalRejectClose}
          >
            Cancel
          </Button>
        </DialogActions>
