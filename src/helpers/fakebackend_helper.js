...
const postJwtLogin = (data) => {
  ...
  return fetch(`${API_URL}${url.POST_FAKE_JWT_LOGIN}`, config)
    .then(response => {
...
// Login Method
export const postJwtRefresh = async () => {
  const params = {
  ...
  if (moment().isAfter(moment(expiresTime))) {
    ...
    return await fetch(`${API_URL}${url.POST_FAKE_JWT_LOGIN}`, config)
      .then(async (response) => {
        if (response.status == 200) {
          ...
          // Token Expire time
          var timestamp = moment().add((data.expires_in / 60) - 10, 'minutes');
